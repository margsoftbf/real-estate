...
export const DashboardContent = () => {
  ...
  const activeProperties = safeProperties.filter(
    (p) => p.isActive === true
  ).length;
  const activeRentals = safeProperties.filter(
    (p) =>
      p.type === PropertyType.RENT &&
      p.isActive === true &&
      p.price &&
  ...
  const averageRentPrice =
    activeRentals.length > 0
      ? Math.round(
          activeRentals.reduce((sum, p) => sum + Number(p.price), 0) /
